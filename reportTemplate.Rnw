...
\ifthenelse{\equal{\Sexpr{input$smoker}}{TRUE}}{
   ...
   An exposure of \Sexpr{round(x.CO_e.fhs()/ppm,1)}$\pm$\Sexpr{round(sd(x.CO_e.fhs.MC())/ppm,1)} ppm to carbon monoxide due to first hand smoke during the exposure phase is %
   ...
   An exposure of \Sexpr{round(x.CO_c.fhs()/ppm,1)}$\pm$\Sexpr{round(sd(x.CO_c.fhs.MC())/ppm,1)} ppm to carbon monoxide due to first hand smoke during the clearance phase is %
